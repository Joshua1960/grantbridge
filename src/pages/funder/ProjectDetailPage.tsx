...
export default function ProjectDetailPage() {
  ...
  const prevSlide = () =>
    setCurrentSlide((p) => (p - 1 + mediaItems.length) % mediaItems.length);
  ...
  if (!pitch) {
    return (
      <div className="min-h-[60vh] flex items-center justify-center">
        <div className="text-center">
          <p className="text-lg font-semibold text-slate-700 font-[Outfit]">
            Project not found
          </p>
          <button
            onClick={() => navigate(-1)}
              ...
              <AnimatePresence mode="wait">
                ...
                >
                  <img
                    ...
                    alt={mediaItems[currentSlide].label}
            ...
            <div className="flex gap-2 mt-3">
              {mediaItems.map((item, i) => (
                <button
                  key={i}
                  onClick={() => setCurrentSlide(i)}
                  className={`relative flex-1 aspect-16/10 rounded-xl overflow-hidden cursor-pointer border-2 transition-all ${i === currentSlide ? "border-brand-500 ring-1 ring-brand-200" : "border-transparent opacity-60 hover:opacity-90"}`}
                >
                  <img
                    src={item.src}
                    alt={item.label}
            ...
            <div className="bg-white rounded-2xl border border-slate-100 p-5 sm:p-6 sticky top-20">
              <div className="mb-5">
                ...
                {pitch.fundedBy ? (
                  <div className="bg-brand-50 rounded-xl p-4 border border-brand-100">
                    <p className="text-[11px] text-brand-600 font-medium mb-1">
                      ✓ Fully Funded
                    </p>
              ...
              <div className="grid grid-cols-3 gap-3 mb-5">
                {[
                  {
                    label: "Views",
                    value: pitch.views.toString(),
                    icon: Users,
                  },
                  { label: "Days Left", value: "28", icon: Clock },
                  {
                    label: "Offers",
                    value: (pitch.offers?.length || 0).toString(),
                    icon: Target,
                  },
                ].map((s) => (
                  <div
                  ...
                  >
                    ...
                    <p className="text-[14px] font-bold text-slate-800 font-[Outfit]">
                      {s.value}
                    </p>
          ...
          <div className="lg:col-span-3 space-y-6">
            ...
            >
              <div className="flex flex-wrap items-center gap-2 mb-3">
                ...
                <span className="px-2.5 py-1 text-[11px] font-semibold bg-brand-50 text-brand-600 rounded-full">
                  {pitch.category}
                </span>
                ...
                <span
                  className={`px-2.5 py-1 text-[11px] font-semibold rounded-full border ${stageColors[pitch.stage]}`}
                >
                  {stageLabels[pitch.stage]}
                </span>
              </div>
              <h1 className="text-xl sm:text-2xl font-bold text-slate-900 font-[Outfit] tracking-tight mb-2">
                {pitch.title}
              </h1>
              <div className="flex items-center gap-4 text-[12px] text-slate-400 mb-5">
                ...
                <span className="flex items-center gap-1">
                  <Eye size={12} /> {pitch.views.toLocaleString()} views
                </span>
                <span className="flex items-center gap-1">
                  <Heart size={12} /> {pitch.likes} likes
                </span>
              </div>
              <div className="flex flex-wrap gap-1.5 mb-5">
                {pitch.tags.map((tag: string) => (
                  <span
                    key={tag}
                    className="px-3 py-1 text-[11px] font-medium bg-slate-50 text-slate-600 rounded-lg border border-slate-100"
                  >
                    {tag}
                  </span>
                ))}
              </div>
              <h2 className="text-[15px] font-semibold text-slate-800 font-[Outfit] mb-3">
                About This Project
              </h2>
              <div className="text-[13px] text-slate-600 leading-relaxed space-y-3">
                <p>{pitch.description}</p>
                <p>
                  Our mission is to create lasting impact by bridging the gap
                  between innovation and capital. We've built a strong
                  foundation with early traction and a clear path to scale. The
                  team brings deep domain expertise and a track record of
                  execution in the {pitch.category.toLowerCase()} space.
                </p>
                <p>
                  With this funding round, we plan to expand our operations
                  across Nigeria, strengthen our technology infrastructure, and
                  enter new markets. Our unit economics are proven and we're
                  ready to accelerate growth significantly.
                </p>
              </div>
            </motion.div>
              ...
              <div className="flex h-4 rounded-full overflow-hidden mb-5">
                {fundBreakdown.map((item, i) => (
                  <motion.div
                    ...
                    className={`${item.color} ${i === 0 ? "rounded-l-full" : ""} ${i === fundBreakdown.length - 1 ? "rounded-r-full" : ""}`}
                  />
                ))}
              </div>
            ...
            <motion.div
              ...
              className="bg-white rounded-2xl border border-slate-100 p-5 sm:p-6"
            >
              <h2 className="text-[15px] font-semibold text-slate-800 font-[Outfit] mb-4">
                Key Metrics
              </h2>
              <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
                {[
                  ...
                  {
                    label: "Customers",
                    value: `${Math.floor(pitch.views * 0.3)}+`,
                    icon: Users,
                  ...
                  {
                    label: "Team Size",
                    value: `${Math.floor(pitch.likes / 50) + 5}`,
                    icon: Users,
                  ...
                  <div
                    key={m.label}
                    className="bg-slate-50 rounded-xl p-4 text-center border border-slate-100/80"
                  >
                    <div
                      className={`w-9 h-9 ${m.color} rounded-lg flex items-center justify-center mx-auto mb-2`}
                    >
                      <m.icon size={16} />
                    </div>
                    <p className="text-[14px] font-bold text-slate-800 font-[Outfit]">
          ...
          <div className="lg:col-span-2 space-y-6">
            ...
            >
              ...
              <div className="flex items-center gap-3 mb-4">
                <img
                  src="/images/entrepreneur-portrait.jpg"
                  alt={pitch.entrepreneurName}
                ...
                <div>
                  ...
                  <p className="text-[11px] text-slate-400 flex items-center gap-1 mt-0.5">
                    <MapPin size={10} /> {pitch.location}, Nigeria
                  </p>
                </div>
              </div>
              <p className="text-[12px] text-slate-500 leading-relaxed mb-4">
                Passionate entrepreneur with 8+ years of experience in{" "}
                {pitch.category.toLowerCase()}. Previously led product teams at
                two successful startups. Committed to creating sustainable
              ...
              <div className="space-y-2.5 mb-4">
                {[
                  ...
                  { icon: Award, label: "Experience", value: "8+ years" },
                  {
                    icon: Globe,
                    label: "Website",
                    value: `${pitch.companyName.toLowerCase().replace(/\s+/g, "")}.com`,
                  },
                ].map((item) => (
                  <div
                    key={item.label}
              ...
              <div className="flex gap-2">
                <button className="flex-1 flex items-center justify-center gap-1.5 py-2.5 bg-brand-50 text-brand-600 rounded-xl text-[12px] font-medium hover:bg-brand-100 transition-colors cursor-pointer">
                  <Mail size={13} /> Message
                </button>
                <button className="flex-1 flex items-center justify-center gap-1.5 py-2.5 bg-slate-50 text-slate-600 rounded-xl text-[12px] font-medium hover:bg-slate-100 transition-colors cursor-pointer">
              ...
              <div className="space-y-3">
                {[
                  ...
                  },
                  {
                    ...
                    desc: "CAC registration confirmed",
                    done: true,
                  },
                  {
                    label: "Pitch Deck Reviewed",
                    desc: "Reviewed by expert panel",
                    done: true,
                  },
                  {
                    label: "Financial Audit",
                    desc: "Third-party financial review",
                    done: pitch.stage !== "idea",
                  },
                ].map((item) => (
                  <div key={item.label} className="flex items-start gap-2.5">
                    ...
                    <div>
                      <p
                        className={`text-[12px] font-medium ${item.done ? "text-slate-700" : "text-slate-400"}`}
                      >
                        {item.label}
                      </p>
            ...
            <motion.div
              ...
              className="bg-white rounded-2xl border border-slate-100 p-5 sm:p-6"
            >
              <h2 className="text-[15px] font-semibold text-slate-800 font-[Outfit] mb-2">
                Discover More Projects
              </h2>
              <p className="text-[12px] text-slate-500 leading-relaxed mb-4">
                Browse other verified projects in the marketplace without making
                another list request on this detail page.
              </p>
              <Link
        ...
        {showShareModal && (
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
            ...
            >
              ...
              <h3 className="text-[16px] font-semibold text-slate-800 font-[Outfit] mb-4">
                Share Project
              </h3>
              <div className="flex items-center gap-2 bg-slate-50 rounded-xl p-2.5 mb-4">
                <input
                  type="text"
                  readOnly
                  value={`grantbridge.com/project/${pitch.id}`}
                  className="flex-1 bg-transparent text-[12px] text-slate-600 outline-none"
                />
                <button
                  onClick={handleCopyLink}
                  className="px-3 py-1.5 bg-brand-500 text-white text-[11px] font-semibold rounded-lg hover:bg-brand-600 transition-colors cursor-pointer flex items-center gap-1"
                >
              ...
              <div className="grid grid-cols-3 gap-2">
                {["Twitter", "LinkedIn", "WhatsApp"].map((p) => (
                  <button
                    key={p}
                    className="py-2.5 bg-slate-50 rounded-xl text-[12px] font-medium text-slate-600 hover:bg-slate-100 transition-colors cursor-pointer"
                  >
                    {p}
                  </button>
                ))}
        ...
        {fundStep !== "closed" && (
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
            <motion.div
              ...
              onClick={isProcessing ? undefined : handleCloseFlow}
            />
            <motion.div
                ...
                <div className="flex items-center gap-1 mb-1">
                  {["amount", "payment", "confirmation"].map((s, i) => {
                    ...
                    return (
                      <div
                        key={s}
                        className={`h-1 flex-1 rounded-full transition-all duration-500 ${i <= currentIdx ? "bg-brand-500" : "bg-slate-200"}`}
                      />
                    );
                  })}
                </div>
                ...
                {fundStep === "amount" && (
                  ...
                  >
                    <div className="text-center mb-5">
                      ...
                      <h3 className="text-[17px] font-bold text-slate-800 font-[Outfit]">
                    ...
                    <p className="text-[10px] text-slate-400 text-center mt-3 flex items-center justify-center gap-1">
                      <Lock size={9} /> Secured by GrantBridge Escrow
                    </p>
                  </motion.div>
                )}
                    ...
                    {!isProcessing ? (
                      <>
                        ...
                        <div className="text-center mb-5">
                          ...
                          <p className="text-[12px] text-slate-400 mt-1">
                            Complete your payment via Flutterwave
                          </p>
                        </div>

                        {/* Payment summary card */}
                        <div className="bg-linear-to-br from-slate-800 to-slate-900 rounded-2xl p-5 mb-5 text-white">
                        ...
                        <div className="space-y-2 mb-5">
                          {[
                            ...
                            <label
                              key={method.label}
                              className={`flex items-center gap-3 p-3.5 rounded-xl border-2 cursor-pointer transition-all ${method.selected ? "border-brand-400 bg-brand-50/50" : "border-slate-100 hover:border-slate-200"}`}
                            >
                              <input
                                type="radio"
                                name="paymethod"
                        ...
                        <div className="flex gap-2">
                          <button
                            onClick={() => setFundStep("amount")}
                            className="px-4 py-3 border border-slate-200 rounded-xl text-[13px] font-medium text-slate-600 hover:bg-slate-50 transition-colors cursor-pointer"
                          >
                            Back
                          </button>
                          <Button
                        ...
                        <p className="text-[10px] text-slate-400 text-center mt-3 flex items-center justify-center gap-1">
                          <ShieldCheck size={9} /> 256-bit SSL encrypted · PCI
                          DSS compliant
                        </p>
                      </>
                    ) : (
                      ...
                      <div className="py-10 text-center">
                        ...
                        />
                        <h3 className="text-[17px] font-bold text-slate-800 font-[Outfit] mb-2">
                          Processing Payment
                        </h3>
                  ...
                  >
                    <div className="text-center py-4">
                      ...
                      >
                        ...
                        <h3 className="text-[20px] font-bold text-slate-900 font-[Outfit] mb-2">
                          Thank You!
                        </h3>
                        ...
                        <div className="space-y-2">
                          ...
                          <button
                            onClick={() => {
                              handleCloseFlow();
                              navigate("/dashboard/funder/discover");
                            }}
                            className="w-full py-2.5 text-[13px] font-medium text-slate-500 hover:text-slate-700 transition-colors cursor-pointer"
                          >
                            Discover More Projects
                          </button>
                        </div>
