...
export default function AccountCreatedPage() {
  ...
  const handleContinue = () => {
    login({
      ...
      fullName: state.fullName || (userRole === 'entrepreneur' ? 'Adaeze Okafor' : 'Sarah Williams'),
      role: userRole,
      ...
      <div className="text-center">
        ...
        >
          ...
          {[...Array(6)].map((_, i) => (
            <motion.div
              ...
              animate={{ opacity: [0, 1, 0], scale: [0, 1, 0.5], x: [0, (i % 2 === 0 ? 1 : -1) * (30 + i * 8)], y: [0, -(20 + i * 10)] }}
        ...
        <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.4, duration: 0.4 }}>
          <div className="inline-flex items-center gap-1.5 px-3 py-1 bg-brand-50 rounded-full mb-3">
            ...
            <span className="text-[11px] font-semibold text-brand-700">Account Verified</span>
          </div>

          <h1 className="text-[24px] font-bold text-slate-900 font-[Outfit] mb-2">Account Created!</h1>
          <p className="text-[13px] text-slate-500 leading-relaxed mb-8 max-w-sm mx-auto">
            Congratulations! Your {userRole} account has been successfully created and verified. You're all set to {userRole === 'entrepreneur' ? 'start pitching your ideas' : 'discover investment opportunities'}.
          </p>
          ...
          <div className="bg-slate-50 rounded-xl p-4 mb-6 text-left space-y-2.5">
            {[
              ...
              { label: 'Role', value: userRole === 'entrepreneur' ? 'Entrepreneur' : 'Funder' },
            ].map((item) => (
          ...
          <Button variant="primary" size="lg" fullWidth onClick={handleContinue} icon={<ArrowRight size={17} />}>
            Go to Dashboard
          </Button>
        </motion.div>
      </div>
