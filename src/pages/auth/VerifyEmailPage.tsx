  ...
  return (
    <AuthLayout role={userRole} variant="verify">
      <div className="text-center">
        ...
        <p className="text-[13px] text-slate-500 mb-1">We've sent a verification link to</p>
        ...
        <div className="bg-slate-50 rounded-xl p-4 mb-6 text-left">
          <p className="text-[12px] text-slate-600 leading-relaxed">
            Click the verification link in the email to confirm your account. The link will expire in <span className="font-semibold text-slate-800">24 hours</span>.
          </p>
          <p className="text-[11px] text-slate-400 mt-2">Can't find the email? Check your spam or junk folder.</p>
        </div>

        <Button variant="primary" size="lg" fullWidth onClick={handleVerified} icon={<ArrowRight size={17} />}>
        ...
        <div className="mt-6 pt-5 border-t border-slate-100">
          <p className="text-[12px] text-slate-400">Wrong email? <Link to={`/signup/${role}`} className="text-brand-600 font-medium hover:underline">Go back to signup</Link></p>
        </div>

        {/* Demo shortcut */}
        <div className="mt-4">
          <button onClick={() => navigate(`/link-expired/${role}`, { state })} className="text-[11px] text-slate-300 hover:text-slate-400 cursor-pointer">
