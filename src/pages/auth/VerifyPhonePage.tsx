  ...
  return (
    <AuthLayout role={userRole} variant="verify">
      <div className="text-center">
        ...
        <h1 className="text-[22px] font-bold text-slate-900 font-[Outfit] mb-2">Verify Phone Number</h1>
        <p className="text-[13px] text-slate-500 mb-1">Enter the 6-digit code sent to</p>
        <p className="text-[14px] font-semibold text-slate-800 mb-6 bg-slate-50 inline-block px-4 py-1.5 rounded-lg">{phone}</p>
          ...
          {otp.map((digit, i) => (
            <motion.input
              ...
              className={`w-12 h-14 text-center text-xl font-bold border-2 rounded-xl outline-none transition-all ${
                digit ? 'border-brand-400 bg-brand-50/50 text-brand-700' : 'border-slate-200 text-slate-800'
        ...
        <div className="flex items-center justify-center gap-1 mb-6 mt-3">
          ...
          <p className="text-[11px] text-slate-400">Code expires in <span className="font-semibold text-slate-500">10:00</span></p>
        </div>
