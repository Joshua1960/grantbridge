...
// export const API_BASE_URL = import.meta.env.DEV ? "/api/v1" : (import.meta.env.VITE_API_BASE_URL?.trim().replace(/\/$/, "") || "/api/v1");
...
//     // FIX: was "/application/json" (typo with leading slash)
...
function extractTokens(response: unknown): Partial<AuthTokens> {
  ...
  return {
    accessToken: asString(
      // FIX: added "access" — backend returns { access, refresh } not { accessToken }
      payload.access ||
        body.access ||
        payload.accessToken ||
        payload.access_token ||
...
async function refreshAccessToken(): Promise<string | null> {
  ...
  refreshPromise = (async () => {
    try {
      // FIX: Django REST framework uses /auth/token/refresh/ with the refresh token in the body
...
function buildAuthResult(
  ...
  if (tokens.accessToken) {
    setAuthTokens(tokens);
    // FIX: also store the refresh token from the response
    const body = asRecord(response);
    const refreshToken = asString(body.refresh);
...
export const api = {
  ...
  getUserPitches: async (userId: string): Promise<PitchCard[]> => {
    // FIX: try /pitches/?user_id=... instead of /users/{id}/pitches
    const response = await apiRequest(
      `/pitches/?user_id=${encodeURIComponent(userId)}`,
    );
    return pickArray(response).map(normalizePitch);
  },
